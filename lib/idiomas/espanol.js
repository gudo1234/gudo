...
const smsRowner = () => { return `> •¡¡Función solo para el owner!!` }//NUMERO DE BOT
const smsOwner = () => { return `> •¡¡Función solo para el owner!!` }//OWNER
const smsMods = () => { return `> •¡¡Función solo para el owner y moderadores!!` }//MODERADORES
const smsPremium = () => { return `> •¡¡Función solo para el owner y usuarios premium!!` }//USUARIOS PREMIUM
const smsGroup = () => { return `\`Este comando solo puede ser usado en un chat grupal\`\n\nUnete https://chat.whatsapp.com/DrhDlBWkWaV5sLOeS9kOBb`  }//PARA GRUPOS
const smsPrivate = () => { return `> •Solo en chats privado del bot¡¡!!` }//AL PRIVADO
const smsAdmin = () => { return `> •¡¡Función solo para los que administran el grupo!!` }//ADMINS
const smsBotAdmin = () => { return `> •¡¡Solo aplico esta función cuando soy admin!!` }//BOT CON ADMIN
const smsUnreg = () => { return `> •Debes registrarte para usar mis comandos, ejemplo: .reg Edar.17` }//VERIFICAR
const smsRestrict = () => { return `> •¡¡Función desactivada por el owner!!` }//RESTRINGIR COMANDO
...
//main.js
const smsWelcome = () => { return '╭─๋︩︪☪︎︎︎̸⃘̸࣭ٜ࣪࣪࣪۬◌⃘۪֟፝֯۫۫︎⃪𐇽۫۬🍧⃘⃪۪֟፝֯۫۫۫۬◌⃘࣭ٜ࣪࣪࣪۬☪︎︎︎︎̸─ׅ─๋︩︪╮\n¡Hola! *@user🍹* buenos días/tardes/noches.\n¡Bienvenido a *@subject*!\n╰─๋︩︪☪︎︎︎̸⃘̸࣭ٜ࣪࣪࣪۬◌⃘۪֟፝֯۫۫︎⃪𐇽۫۬🍧⃘⃪۪֟፝֯۫۫۫۬◌⃘࣭ٜ࣪࣪࣪۬☪︎︎︎︎̸─ׅ─๋︩︪╯\n\n\`Disfruta del grupo, diviértete, no olvides en leer las reglas...\`'}
const smsBye = () => { return '*@user* salió del grupo...'}
...
//_allantilink.js
const smsTextoYT = () => { return 'wamods '} 
const smsApagar = () => { return '❌ DESACTIVAR'} 
const smsEncender = () => { return '✅ ACTIVAR'} 
const smsEnlaceTik = () => { return `ban`}
const smsEnlaceYt = () => { return `ban`}
const smsEnlaceTel = () => { return `ban`}
const smsEnlaceFb = () => { return `ban`}
const smsEnlaceIg = () => { return `ban`}
const smsEnlaceTw = () => { return `ban`}
const smsAllAdmin = () => { return `> •Función dolo aplica cuando soy admin`}
const smsSoloOwner = () => { return `> •Activar: .on restrict`}
...
const smsCont17 = () => { return `\`Sistema de block🔐\`\n\n`}
const smsCont18 = () => { return `Las llamadas no están permitidas, comunicate con el owner +50492280729 si necesitas algo.`}
...
//_antilink.js
const smsAdwa = () => { return `${global.lenguajeGB['smsAvisoEG']()}> •Admin detectado, no serás eliminado.`}
const smsEnlaceWat = () => { return `${lenguajeGB['smsAvisoAG']()}ban`}

//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeGB['smsAvisoAG']()}ban`}
...
const eGataCoins = () => { return '🐱 KilluaCoins' }
const eGataTickers = () => { return '🎫 Killua Tickers' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar Izumi-Bot`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar Izumi-Bot en Grupos`}
...
//jadibot-serbot.js
const smsIniJadi = () => { return `> Escanea...`}
const smsSoloOwnerJB = () => { return `${lenguajeGB['smsAvisoAG']()}*ESTE COMANDO ESTÁ DESACTIVADO POR MÍ PROPIETARIO(A)*`}
const smsJBPrincipal = () => { return `_Número principal del bot_`}
const smsIniJadi2 = () => { return `> Vincilar con el número de teléfono.`}
