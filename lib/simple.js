      ...
      */
      ...
      async value(jid, text = '', quoted, options) {
        if (Buffer.isBuffer(text)) {
          ...
          let canalId = ["120363285614743024@newsletter", "120363285614743024@newsletter", "120363285614743024@newsletter", "120363285614743024@newsletter"]
          let canalNombre = ["꙳🧧𓆩ίʑ᭘ɱί-ⲃⲟτ𓆪🧧꙳", "꙳🧧𓆩ίʑ᭘ɱί-ⲃⲟτ𓆪🧧꙳", "꙳🧧𓆩ίʑ᭘ɱί-ⲃⲟτ𓆪🧧꙳", "꙳🧧𓆩ίʑ᭘ɱί-ⲃⲟτ𓆪🧧꙳"]
...
sendbutton4 = async (jid, button = [], opts = {}, quoted = {}, options = {}) => {
    if (opts.media) {
        let prepare;
        const file = await functions.getFile(opts.media);
        const buffer = fs.readFileSync(file.file);

        if (/image/.test(file.mime)) {
            const parsed = await prepareWAMessageMedia({ image: buffer }, { upload: conn.waUploadToServer });
            prepare = { imageMessage: parsed.imageMessage };
        } else if (/video/.test(file.mime)) {
            const parsed = await prepareWAMessageMedia(
                { video: buffer, ...(opts.gif && { gifPlayback: true }) },
                { upload: conn.waUploadToServer }
            );
            prepare = { videoMessage: parsed.videoMessage };
        } else if (/document/.test(file.mime)) {
            const parsed = await prepareWAMessageMedia(
                {
                    document: buffer,
                    mimetype: 'application/pdf', // Cambia según el tipo
                    fileName: opts?.fname || 'Archivo',
                },
                { upload: conn.waUploadToServer }
            );
            prepare = { documentMessage: parsed.documentMessage };
        } else {
            prepare = {};
        }
                ...
                viewOnceMessage: {
                    message: {
                        buttonsMessage: {
                            ...
                            buttons: button,
                            headerType: prepare?.imageMessage
                                ? 4
                                : prepare?.videoMessage
                                ? 5
                                : prepare?.documentMessage
                                ? 3
                                : 1,
                        },
                    },
        ...
        return conn.relayMessage(jid, message.message, { messageId: message.key.id });
    }
}
