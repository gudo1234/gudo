...
if (!text) return conn.reply(m.chat,  `${e} _Ejemplo ${usedPrefix + command} bad bunny_`, m, {contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: wm, previewType: 0, thumbnail: img.getRandom(), sourceUrl: redes.getRandom()}}})
try {
let resultados = await spotifyxv(text)
let res = resultados.map((v, i) => {
let duracion = timestamp(v.duracion)
//return `Descargue con .spotifydl url`
...
await conn.reply(m.chat, `Error intente de nuevo`, m)
console.log(`error`)
console.log(e)
}}
