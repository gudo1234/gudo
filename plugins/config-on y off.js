...
break
default:
if (!/[01]/.test(command)) return await conn.reply(m.chat, `   \`OPCIONES PARA ACTIVAR/DESACTIVAR\`
...
* ${usedPrefix + command} modohorny ${m.isGroup ? chat.modohorny ? '✅' : '❌' : lenguajeGB.smsNoGg()} 
...
* ${usedPrefix + command} delete ${m.isGroup ? chat.delete ? '✅' : '❌' : lenguajeGB.smsNoGg()}
...
> ${lenguajeGB.smsAntiE2()}`, fkontak, { mentions: [aa,] })
throw false
}
await conn.reply(m.chat, `*Opción:* ${type}\n*Estado:* ${isEnable ? 'Activado' : 'Desactivado'}\n*Para:* ${isAll ? 'Este bot' : isUser ? '' : 'Este chat'}`, fkontak, {
