...
const ddownr = {
  download: async (url, format) => {
    if (!formatAudio.includes(format) && !formatVideo.includes(format)) {
      throw new Error(`${e} Formato no soportado, verifica la lista de formatos disponibles.`);
    }
...
// Para video🗿
      ...
      for (let source of sources) {
        try {
          ...
          if (downloadUrl) {
            success = true;
            await conn.sendMessage(m.chat, { document: { url: downloadUrl }, mimetype: "video/mp4", fileName: `video.mp4`, caption: `${title}` }, { quoted: m });
            break;
          }
        } catch (e) {
...
handler.command = handler.help = ['play4', 'ytvdoc', 'play2doc', 'ytmp4doc']; 
