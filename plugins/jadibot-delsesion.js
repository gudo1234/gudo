...
return conn.sendMessage(m.chat, {text: `${e} *USE ESTE COMANDO AL BOT PRINCIPAL*\n\nwa.me/${global.conn.user.jid.split`@`[0]}&text=${usedPrefix + command}`}, { quoted: m }) 
} else {
try {
await fs.rmdir("./jadibts/" + uniqid, { recursive: true, force: true })
await conn.sendMessage(m.chat, { text: `${e} Se borró la sesion` }, { quoted: m })
await conn.sendMessage(m.chat, { text : `${e} *HA CERRADO SESIÓN Y BORRADO TODO RASTRO*` } , { quoted: m })
} catch(err) {
