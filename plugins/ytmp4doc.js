...
if (size.split('MB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play4\``, m, rcanal).then(_ => m.react('✖️'))
if (size.split('GB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play4\``, m, rcanal).then(_ => m.react('✖️'))
...
if (size.split('MB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play3\``, m, rcanal).then(_ => m.react('✖️'))
if (size.split('GB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play4\``, m, rcanal).then(_ => m.react('✖️'))
	let txt = '`乂  Y O U T U B E  -  M P 4 D O C`\n\n'
       txt += `	✩   *Titulo* : ${title}\n`
       txt += `	✩   *Calidad* : ${q}\n`
