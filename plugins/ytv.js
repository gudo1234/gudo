...
if (sizeMB.split('MB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play2\``, m, rcanal).then(_ => m.react('✖️'))
...
if (size.split('MB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se canceló la Descarga. \`Prueba con ${usedPrefix}play2\``, m, rcanal).then(_ => m.react('✖️'))
	let txt = '`乂  Y O U T U B E  -  M P 4`\n\n'
       txt += `	✩   *Titulo* : ${title}\n`
       txt += `	✩   *Calidad* : ${q}\n`
       txt += `	✩   *Tamaño* : ${size}\n\n`
...
handler.command = ['ytv2', 'ytmp42', 'mp42']
handler.group = true;
//handler.limit = 1
